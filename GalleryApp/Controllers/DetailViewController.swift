...
class DetailViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func configureDate() {
        ...
        formater.locale = Locale(identifier: AppStrings.localeOfFormatter.rawValue.localised)
        formater.dateFormat = "d MMMM yyyy"
        self.title = formater.string(from: date)
    }

    func configureBackButton() {
        let backButton = UIBarButtonItem(image: UIImage.AppIcons.chevronLeftIcon, style: .plain, target: self, action: #selector(back))
    ...
    func presentActivityViewController(image: UIImage) {
        ...
        activityViewController.completionWithItemsHandler = { type, isSuccessful, _, _ in
            if (type == .saveToCameraRoll) {
                ...
                if (isSuccessful) {
                    titleText = AppStrings.photoWasSaved.rawValue.localised
                } else {
                    titleText = AppStrings.photoWasNotSaved.rawValue.localised
                }
                let alert = UIAlertController(title: titleText, message: nil, preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: AppStrings.accept.rawValue.localised, style: .cancel, handler: { _ in
